...
try {
  ...
  const generationId = ownMessages.messages[1].request_id
  assert.match(generationId, /^[0-9a-f-]{36}$/)
  assert.equal((await request(`${messagesPath}/stop`, b, 'POST', { messageId: assistantId, requestId: generationId })).status, 404)
  assert.equal((await request(`${messagesPath}/stop`, a, 'POST', { messageId: assistantId })).status, 400, 'A Stop without a generation identity must not mutate a reused message')
  assert.equal((await request(`${messagesPath}/stop`, a, 'POST', { messageId: assistantId, requestId: generationId })).status, 200)
  assert.equal((await (await request(messagesPath, a)).json()).messages.at(-1).status, 'interrupted')
  ...
  assert.equal((await request(`${messagesPath}/stop`, a, 'POST', { messageId: assistantId, requestId: generationId })).status, 200)
  const afterLateStop = (await (await request(messagesPath, a)).json()).messages.at(-1)
  assert.equal(afterLateStop.status, 'pending', 'A delayed Stop must not interrupt the newer generation')
  assert.equal(afterLateStop.request_id, retryRequestId)
  ...
  assert.equal((await request(`${messagesPath}/stop`, a, 'POST', { messageId: assistantId, requestId: retryRequestId })).status, 200)
  assert.equal((await (await request(messagesPath, a)).json()).messages.at(-1).status, 'complete', 'Stop must not rewrite an already completed response')
