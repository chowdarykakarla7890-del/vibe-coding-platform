...
import { checkArchiveImports } from './check-archive-imports.mjs'
...
async function account() {
  const email = `archive-import-${randomUUID()}@example.invalid`, password = randomBytes(24).toString('hex')
...
try {
  ...
  await checkArchiveImports({ admin, request, a, b })
} finally {
  ...
  console.log('Disposable archive-import accounts and projects removed; temporary sessions signed out.')
}
